...
module.exports = async function getUser() {
	...
	try {
		...
		const overall = await eleventyFetch(
			`http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${process.env.STEAM_API_KEY}&steamid=76561198121851448&include_played_free_games=true&format=json`,
			{
				duration: "1d",
				type: "json",
			}
		...
		const indiv = await eleventyFetch(
			`http://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v0002/?appid=440&key=${process.env.STEAM_API_KEY}&steamid=76561198121851448&format=json`,
			{
				duration: "1d",
				type: "json",
			}
